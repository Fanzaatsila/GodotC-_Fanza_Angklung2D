...
public static class GraphicsUtils
{
    ...
    public static void PutPixelAllFast(Node2D targetNode, List<Vector2> dot, DrawStyle style = DrawStyle.DotDot, Godot.Color? color = null, int stripLength = 3, int gap = 0)
    {
        // 1. Use ScreenWidth and ScreenHeight from ScreenUtils
        bool[,] pixels = new bool[ScreenUtils.ScreenWidth, ScreenUtils.ScreenHeight];

        // 2. "Draw" into the array by setting corresponding elements to true
        for (int i = 0; i < dot.Count; i++)
        {
            int x = (int)dot[i][0];
            int y = (int)dot[i][1];

            // Bounds checking using ScreenWidth and ScreenHeight
            if (x >= 0 && x < ScreenUtils.ScreenWidth && y >= 0 && y < ScreenUtils.ScreenHeight)
            {
                switch (style)
                {
                    case DrawStyle.DotDot:
                    ...
                    case DrawStyle.StripStrip:
                        if (i % (gap + 1) == 0)
                        {
                            ...
                            if (style == DrawStyle.DotStripDot || style == DrawStyle.StripStrip)
                            {
                                ...
                                for (int j = -halfLength; j <= halfLength; j++)
                                {
                                    if (x + j >= 0 && x + j < ScreenUtils.ScreenWidth) // Bounds check for strip
                                    {
                                        pixels[x + j, y] = true;
                                    }
                                }
                    ...
                    case DrawStyle.CircleDotStrip:
                        if ((i / 8) % (gap + 1) == 0)
                        {
                            ...
                            if (style == DrawStyle.CircleDotStrip)
                            {
                                int halfLength = stripLength / 2;
                                for (int j = -halfLength; j <= halfLength; j += gap + 1)
                                {
                    ...
                    case DrawStyle.EllipseDotStrip:
                        if ((i / 4) % (gap + 1) == 0)
                        {
                            ...
                            if (style == DrawStyle.EllipseDotStrip)
                            {
                                int halfLength = stripLength / 2;
                                for (int j = -halfLength; j <= halfLength; j += gap + 1)
                                {
        ...
        // 3. Iterate through the array and call PutPixel for true elements
        for (int x = 0; x < ScreenUtils.ScreenWidth; x++)
        {
            for (int y = 0; y < ScreenUtils.ScreenHeight; y++)
            {
                if (pixels[x, y])
                {
                    PutPixel(targetNode, x, y, color);
